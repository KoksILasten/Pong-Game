...
use ggez::nalgebra::{self as na, ComplexField};
...
fn move_racket(pos: &mut na::Point2<f32>, keycode: KeyCode, y_dir: f32, ctx: &mut Context){
    ...
    clamp( &mut pos.y, 0.0 + RACKET_HEIGHT_HALF, screen_h - RACKET_HEIGHT_HALF);
...
impl event::EventHandler for MainState {
    fn update(&mut self, ctx: &mut Context) -> GameResult<()>{
        ...
        self.ball_pos += self.ball_vel * delta_time;
    ...
    fn draw(&mut self, ctx: &mut Context) -> GameResult<()> {
        ...
        let (score_texte_w, score_text_h) = score_text.dimensions(ctx); //gets text width and heigh
        score_pos -= na::Vector2::new(score_texte_w as f32 * 0.5, score_text_h as f32 * 0.5); //scoreboard aligment
...
fn main() -> GameResult {
    let cb = ggez::ContextBuilder::new("Pong_0", "Jeff");
    let (ctx, event_loop) = &mut cb.build()?;
    
    // ctx.gfx.set_window_title("Pong game"); Too new
    graphics::set_window_title(ctx, "Pong game"); //depreciated
